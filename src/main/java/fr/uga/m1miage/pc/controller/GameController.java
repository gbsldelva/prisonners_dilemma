...
public class GameController {
    ...
    public void makeChoice(@Payload ChoiceMessage choiceMessage) {
        ...
        if (session != null) {
            Player player = session.getPlayer1().getUsername().equals(choiceMessage.getUsername()) ? session.getPlayer1() : session.getPlayer2();

            if (player.getStrategy() != null) {
                String strategyMove = player.getStrategy().decideMove(session, player);
                session.addChoice(player, strategyMove);
            } else {
                session.addChoice(player, choiceMessage.getChoice());
            }
            if (session.getPlayer1Choices().size() == session.getPlayer2Choices().size()) {
